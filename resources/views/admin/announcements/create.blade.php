...
<div class="container-fluid">
    ...
    <div class="announcements-header">
        <div class="d-flex align-items-center justify-content-between">
            <div>
                <h4><i class="bx bx-plus me-2"></i>Create Announcement</h4>
                <p>Create a new announcement, banner, or notification for users</p>
            </div>
            <div>
    ...
    <div class="form-container">
        <div class="card-header">
            ...
            <p>Fill in the details for your new announcement</p>
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.announcements.store') }}" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="display_type" id="display_type" class="form-select @error('display_type') is-invalid @enderror" required>
                            ...
                            <option value="notification" {{ old('display_type') == 'notification' ? 'selected' : '' }}>Notification</option>
                        </select>
                        @error('display_type')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="audience_type" id="audience_type" class="form-select @error('audience_type') is-invalid @enderror" required>
                            ...
                            <option value="roles" {{ old('audience_type') == 'roles' ? 'selected' : '' }}>Specific Roles</option>
                            <option value="users" {{ old('audience_type') == 'users' ? 'selected' : '' }}>Specific Users</option>
                        </select>
                        @error('audience_type')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                ...
                <div id="audience-roles" class="audience-section {{ old('audience_type') == 'roles' ? '' : 'hidden' }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-check">
                                <input type="checkbox" 
                                       ...
                                       {{ in_array($role, old('audience_payload', [])) ? 'checked' : '' }}>
                ...
                <div id="audience-users" class="audience-section {{ old('audience_type') == 'users' ? '' : 'hidden' }}">
                    <label class="form-label">Select Users</label>
                    <select name="audience_users[]" id="audience_users" class="form-select" multiple>
                        @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ in_array($user->id, old('audience_users', [])) ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    <button type="submit" class="btn btn-primary w-100">
                        <i class="bx bx-save me-2"></i>Create Announcement
                    </button>
                </div>
            </form>
        </div>
...
$(document).ready(function() {
    ...
    if (!$('#end_date').val()) {
        $('#end_date').val(nextWeek.toISOString().slice(0, 16));
    }
});
</script>
@endsection
